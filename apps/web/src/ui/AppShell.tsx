import { BarChart3, FileText, Globe2, ListChecks, Settings, Users } from "lucide-react";
        ...
        <header className="sticky top-0 z-10 border-b border-slate-200 bg-white/90 px-5 py-4 backdrop-blur">
          <div className="flex flex-wrap items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <button className="rounded-md border border-slate-200 px-3 py-2 text-sm text-slate-600" disabled={logout.isPending} onClick={() => logout.mutate()}>
                خروج
              </button>
            </div>
          </div>
  ...
  return (
    <main className="flex min-h-screen items-center justify-center bg-mist px-5 text-right text-ink" dir="rtl">
      <form onSubmit={form.handleSubmit((values) => login.mutate(values))} noValidate className="w-full max-w-md rounded-lg border border-slate-200 bg-white p-6">
        ...
        <button className="mt-5 w-full rounded-md bg-teal px-4 py-2 text-sm font-semibold text-white" disabled={login.isPending}>
